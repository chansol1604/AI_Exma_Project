...
# Conv2DTranspose = Conv2D의 반대 동작, 오히려 데이터의 크기가 커짐, (14, 14, 256)
generator.add(Conv2DTranspose(128, kernel_size=3, strides=2, padding='same'))   #strides -> 커널이 움직이는 칸 수
generator.add(BatchNormalization())      # 값이 커지기 때문에 삽입
generator.add(LeakyReLU(alpha=0.01))
generator.add(Conv2DTranspose(64, kernel_size=3, strides=1, padding='same'))    #(14, 14, 256)
generator.add(BatchNormalization())
generator.add(LeakyReLU(alpha=0.01))
generator.add(Conv2DTranspose(1, kernel_size=3, strides=2, padding='same'))     #(28, 28, 256)
...
for epochs in range(epochs):
    ...
    if epochs % sample_interval == 0:
        ...
        path = os.path.join(OUT_DIR, 'img_num4-{}'.format(epochs+1))
        plt.savefig(path)
        plt.close()
        generator.save('./models/generator_4_LCS.h5')