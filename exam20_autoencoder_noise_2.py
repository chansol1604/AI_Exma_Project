...
x = Conv2D(16, (3, 3), activation='relu')(x)                      # 14 * 14   사이즈 조절을 위해 padding='same' 주지 않음.
...
conv_x_train = x_train.reshape(-1, 28, 28, 1)   # (개수, 가로, 세로, 칼러)
...
fit_hist = autoencoder.fit(conv_x_train_noisy, conv_x_train,
       epochs=100, batch_size=256, validation_data=(conv_x_test_noisy, conv_x_test))
